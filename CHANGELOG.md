...
- added a "Broken files repaired" strip to the Overview — sits as a full-width sibling BELOW the reclaim hero (not inside its grid, so the two reclaim panels above spread across the full row instead of getting squeezed narrower to make room for a third grid slot). Layout: lifetime headline stack on the left (big mono count · "X GB saved · lifetime"), then four rung tiles side-by-side on the right — one per ladder step, always shown even at zero so users see the whole ladder. Each tile has its own 3px colored accent bar (purple / amber / cyan / pink for remux / drop / MKV coerce / re-encode), big number + saved-bytes hint, label underneath. Zero-count tiles dim to 55% opacity. Deliberately not a donut — the "Reclaimed since install" panel above already owns that visual, using it twice on the same page dilutes the meaning. Hidden on a fresh install (`CountLifetime === 0`). Pre-repair source size is captured at rescue time into a new `history.saved_bytes` column (schema v7 migration; existing rows read as 0) so the GB metric reflects real files-that-would-have-been-deleted.
- raw ffmpeg / ffprobe error dumps no longer bleed into the History-tab, Recycle-bin-tab, or Issues-tab primary text. Fix messages, action strings, and "Why:" one-liners now stay plain-language ("Removed unplayable file X — The video stream is damaged and the decoder rejected part of it (kept in recycle bin).", "Rebuilding the file failed; the original is untouched.", etc.) and the raw multi-line ffmpeg / exception text is tucked into a collapsible **Technical detail** disclosure directly under the row. The Recycle-bin chip for unplayable files also shortens from "Unplayable file removed" to **Damaged file**. Every user-visible fixer message across PlayabilityFixer, TrackFixer, TranscodeFixer, EmbeddedCoverArtFixer, SubtitleFontFixer, NfoFixer, OrphanCleanupFixer, MediaGrouperFixer, and the FixTask exception handlers is affected; the raw stderr / exception message is preserved so troubleshooting and support tickets still have the technical context. Persisted in a new `history.action_detail` column (schema v8 migration; existing rows read NULL so they render as before, minus the ffmpeg dump — pre-migration rows keep whatever inline text they were stored with).
